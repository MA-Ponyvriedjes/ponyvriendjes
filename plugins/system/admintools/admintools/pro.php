...
class plgSystemAdmintoolsPro extends JPlugin
{
	...
	public function onAfterInitialise()
	{
		...
		// Front-end stuff
		{
			// GZip header cloaking
			$encodedby = $this->cparams->getValue('encodedby','');
			if( !empty($encodedby) ) {
				if(!class_exists('JResponse')) {
					// Load our own JResponse class which allows overriding headers
					require_once 'response.php';
				}
				// Set the GZIP header
				JResponse::setHeader('X-Content-Encoded-By', $encodedby, true);
			}
			...
			if(!$this->skipFiltering) {
				...
				if($this->cparams->getValue('tpone',0) == 1) $this->disableTpOne();
	...
	{
		$db = JFactory::getDBO();
		$db->setQuery('SELECT `word` FROM `#__admintools_badwords` GROUP BY `word`');
	...
	private function customRouter()
	{
		...
		$db = JFactory::getDBO();
		$query = 'SELECT `source` FROM `#__admintools_redirects` WHERE (`dest` = '.
			$db->Quote($path).' OR `dest` = '.$db->Quote($fullurl).
			') AND `published` = 1 ORDER BY `ordering` DESC LIMIT 0,1';
		$db->setQuery($query);
		...
		if(in_array($app->getName(),array('administrator','admin')))
		{
			if($user->guest)
			{
				...
				if(($option=='com_login') && ($task=='login'))
				{
					...
					// request.
					return !JRequest::checkToken('request');
				}
				else
				{
	...
	private function logBreaches($reason, $extraLogInformation = '', $extraLogTableInformation = '')
	{
		...
		if(!empty($safeIPs)) {
			...
			if(!empty($safeIPs)) foreach($safeIPs as $safeIP) {
				$safeIP = trim($safeIP);
				if($ip == $safeIP) return false;
			}
		}
		...
		if($this->cparams->getValue('logbreaches',0))
		{
			...
			$db = JFactory::getDBO();
			$query = 'INSERT INTO `#__admintools_log` (`logdate`,`ip`,`url`, `reason`, `extradata`) '.
				'VALUES ('.$db->Quote($date->toMySQL()).', '.
				$db->Quote($ip).', '.$db->Quote($url).', '.$db->Quote($reason).', '.
				$db->Quote($extraLogTableInformation).')';
			$db->setQuery($query);
			$db->query();
		}
	 ...
	 */
	private function emailOnFailedAdminLogin()
	{
	...
	private function CSRFShield_ADVANCED()
	{
		...
		// Run the basic (referer) filtering
		$this->CSRFShield_BASIC();
	...
	private function ProjectHoneypotHTTPBL()
	{
		...
		// Get the IP address
		$reqip = htmlspecialchars($_SERVER['REMOTE_ADDR']);
		if (strpos($reqip, '::') === 0) {
...
ENDINFO;
	...
	private function UploadShield()
	{
		...
		foreach($filesHash as $key => $descriptor) {
			$tempName = $descriptor['tmp_name'];
			$intendedName = $descriptor['name'];
			
			$extraInfo .= "File descriptor :\n";
			$extraInfo .= print_r($descriptor, true);
			$extraInfo .= "\n";
