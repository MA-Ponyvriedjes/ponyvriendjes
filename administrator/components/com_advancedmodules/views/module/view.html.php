...
class AdvancedModulesViewModule extends JView
{
	...
	protected function getConfig()
	{
		if ( !isset( $this->config ) ) {
			...
			$this->config = $parameters->getParams( $config->toObject(), JPATH_ADMINISTRATOR.'/components/com_advancedmodules/config.xml' );
		}
		return $this->config;
	}
	...
	protected function addToolbar()
	{
		...
		if ( $canDo->get( 'core.admin' ) ) {
			...
			$parameters = NNParameters::getInstance();
			$config = JComponentHelper::getParams( 'com_advancedmodules' );
			$config = $parameters->getParams( $config->toObject(), JPATH_ADMINISTRATOR.'/components/com_advancedmodules/config.xml' );
