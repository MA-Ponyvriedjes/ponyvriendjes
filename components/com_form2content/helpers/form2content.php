...
class Form2ContentHelper
{
	...
	public static function filterUserUtcWithFormat($value)
	{
		if (intval($value) > 0) 
		{
			...
			{
				JFactory::getApplication()->enqueueMessage(sprintf(JText::_('COM_FORM2CONTENT_ERROR_DATE_FIELD_INCORRECT_DATE_ENTERED'), $translatedDateFormat), 'Warning');
				return '';				
			}
			...
			$dateWithOffset = new JDate($date->toMySQL(), $offset);
			
			$return = $dateWithOffset->toMySQL();
		}
		else 
		{
			$return = '';
