...
class Path
{
	function Combine($path, $fileName)
	{
...
{
	function syncArticleOrder($catid)
	{
		$db		=& JFactory::getDBO();
		
		$sql =	'UPDATE #__content c INNER JOIN #__f2c_form f ON f.reference_id = c.id ' .
				'SET c.ordering = f.ordering WHERE f.catid = ' . (int)$catid;
		$db->setQuery($sql);
		$db->query();
...
{
	function generateSampleTemplate($contentTypeId)
	{
		$db =& JFactory::getDBO();
		...
		for ($i=0, $n=count($rows); $i < $n; $i++) 
	    {
	      	...
	      	switch(intval($row->fieldtypeid))
	      	{
	      		...
	      		case F2C_FIELDTYPE_MULTISELECTLIST:
  					$template .= $row->fieldname.': <ul>{$'.strtoupper($row->fieldname)."}</ul>\n";			
	      			break;

	      		case F2C_FIELDTYPE_FILE:
	      			$template .= $row->fieldname.' (url): {$'.strtoupper($row->fieldname)."}<br/>\n";			
	      			$template .= $row->fieldname.' (filename): {$'.strtoupper($row->fieldname)."_FILENAME}\n";			
	      			break;
	      			
	      		case F2C_FIELDTYPE_GEOCODER:
	      			$template .= $row->fieldname.' (addres): {$'.strtoupper($row->fieldname)."_ADDRESS}<br/>\n";			
	      			$template .= $row->fieldname.' (latitude): {$'.strtoupper($row->fieldname)."_LAT}<br/>\n";			
	      			$template .= $row->fieldname.' (longitude): {$'.strtoupper($row->fieldname)."_LON}\n";			
	      			break;
